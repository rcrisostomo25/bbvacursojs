...
class Utilitario {
	...
	static generarNombreEquipoAleatorio(){
	    var nombresAutores = ["Real Madrid", "Barcelona", "Atletico Madrid", "Dep. la coruña", "Malaga", "Sevilla", "Chelsea", "Bayenr Munich", "Man. City", "Man. United"];
	    var indice = this.generarNumeroAleatorioEntre(0, nombresAutores.length-1);

	    return nombresAutores[indice];
	}

	static generarNombreEquipoPorPosicion(indice){
	    var nombresAutores = ["Real Madrid", "Barcelona", "Atletico Madrid", "Dep. la coruña", "Malaga", "Sevilla", "Chelsea", "Bayenr Munich", "Man. City", "Man. United"];
	    return nombresAutores[indice];
	}
}
...
class Equipo {
	constructor(entrenador, indice) {
		...
		this._nombre = Utilitario.generarNombreEquipoPorPosicion(indice);
		this._puntaje = 0;
	...
	constructor(equipo1, equipo2) {
 	...
 	jugarPartido(iteraciones) {
 		...
 		if(this._equipo1._goles == this._equipo2._goles){
 			this._equipo1._partidosEmpatados++;
 			this._equipo1._puntaje += 1;

 			this._equipo2._partidosEmpatados++;
 			this._equipo2._puntaje += 1;

 		} else if(this._equipo1._goles > this._equipo2._goles){
 			this._equipo1._partidosGanamos++;
 			this._equipo1._puntaje += 3;

 		} else {
 			this._equipo2._partidosGanamos++;
 			this._equipo2._puntaje += 3;
 		}

 		this._equipo1._goles = 0;
 		this._equipo2._goles = 0;
 	}
...
class Torneo {
	...
	crearEquiposParticipantes(cantidadEquipos) {
		for(let indiceEquipo=0; indiceEquipo<cantidadEquipos; indiceEquipo++) {
			let entrenadorAleatorio = EntrenadoAleatorio.obtenerEntrenadorAleatorio();
			let equipo = new Equipo(entrenadorAleatorio,indiceEquipo);
			equipo.crearPlantilla();
			entrenadorAleatorio.elegirPlantillaParaPartido(equipo);
	...
	empezarTorneo() {
		for(let e1=0; e1<this._equipos.length; e1++) {
			var equipo1 = this._equipos[e1];

			for(let e2=0; e2<this._equipos.length; e2++) {
				var equipo2 = this._equipos[e2];

				if(equipo1._id != equipo2._id) {
					var partido = new Partido(equipo1, equipo2);
					partido.jugarPartido(10);
				}
			}
		}	
	}
	...
	resultadosTorneo() {
		this._equipos.sort(function(a, b) {
		    return parseFloat(b._puntaje) - parseFloat(a._puntaje);
		});

		console.log("::::::: TABLA DE POSICIONES ::::::: ")
		for(let e2=0; e2<this._equipos.length; e2++) {
			var equipo2 = this._equipos[e2];
			console.log("  ** " + equipo2._nombre);
			console.log("  PUNTAJE: " + equipo2._puntaje + " // PARTIDOS GANADOS: " + equipo2._partidosGanamos + " // PARTIDOS EMPATADOS: " + equipo2._partidosEmpatados);
			console.log("--------------------------------------------------------------------")
		}
	}
} 
...
class EntrenadoAleatorio {
	constructor() {

	}

	static obtenerEntrenadorAleatorio() {
		var entrenador = Math.random() > 0.5 ? new EntrenadoDefensivo() : new EntrenadoAtacante();
		return entrenador;
	}
}
...
torneo.crearEquiposParticipantes(10);
torneo.empezarTorneo();
torneo.resultadosTorneo();
